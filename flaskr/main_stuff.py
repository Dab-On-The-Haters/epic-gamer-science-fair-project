...
def newDataset():
    ...
    if DF.newURL.data:
        ...
        if invalidFiles:
            return render_template('new-dataset.html', form=DF, invalidFiles=invalidFiles, user=current_user)
...
def modelMaker():
    ...
    return render_template('model-maker.html', advancedIDs=[MF.layerAmount.id, MF.dropout.id, MF.seqLength.id, MF.seed.id, MF.learningRate.id], form=MF, user=current_user)
